$(document).ready(function () {
	...
	const startGame = () => {
		...
		heroSelection = [knight,elf,orc,wizard];
		...
		$.each(heroSelection, function(index, hero) {
			
			var newheroDiv = $("<div>").addClass("hero panel panel-success").attr("id",hero.id);

			$("<div>").addClass("panel-heading").html(hero.name).appendTo(newheroDiv);
			$("<div>").addClass("panel-body").append("<img src='" + hero.img + "'>").appendTo(newheroDiv);
			$("<div>").addClass("panel-footer").append("<span class='hp'>" + hero.healthPoints + "</span>").appendTo(newheroDiv);

			$("#heroSelection").append(newheroDiv);
		});

		$(".hero").on("click", function() {
			
			if(hero === null) {
				console.log("picked hero");
				
				var charId = parseInt($(this).attr("id"));

				hero = heroSelection[charId];

				$.each(heroSelection, function(index, hero) {
					
					if(hero.id !== charId) {
						opponents.push(hero);
						$("#"+hero.id).removeClass("hero panel-success").addClass("opponent panel-danger").appendTo("#opponentArea");
					} else {
						$("#"+hero.id).appendTo("#hero");
					}
				});
				...
				$(".opponent").on("click", function() {
					if(opponent === null) {
						...
						console.log(this);
						opponent = heroSelection[opponentId];
	...
	$("#attack").on("click", function() {
		...
		if(hero !== null && hero.healthPoints > 0 && opponents.length > 0) {
			...
			if(opponent !== null) {
				...
				status += "You Attacked The " + opponent.name + " For " + hero.attackPower + " Damage.<br>";

				console.log("opponent: ",opponent.name,opponent.healthPoints);

				$("#"+opponent.id + " .hp").html(opponent.healthPoints);

				hero.healthPoints -= opponent.counterAttackPower;
				status += "The " + opponent.name + " Attacked You Back For " + opponent.counterAttackPower + " Damage.<br>";

				console.log("hero: ",hero.name,hero.healthPoints);

				$("#"+hero.id + " .hp").html(hero.healthPoints);

				hero.attackPower += hero.baseAttack;

				if(hero.healthPoints <= 0) {
					status = "You Have Been Defeated... GAME OVER!!!!";
					$("#restartButton").show();
				} else if(opponent.healthPoints <= 0) {
					status = "You Have Defeated The " + opponent.name + ", Please Choose Another Enemy To Continue.";
				...
				if(opponents.length === 0) {
					status = "Congratulations You Have Won The Battle!";
					$("#restartButton").show();
				}
			} else {
				status = "Please Select An Enemy To Continue.";
			}
